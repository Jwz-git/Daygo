...
func (r *ProviderRepo) List(ctx context.Context) ([]Provider, error) {
	...
	err := r.store.Read(ctx, "providers list", func(ctx context.Context, tx *sql.Tx) error {
		rows, err := tx.QueryContext(ctx,
			`SELECT id, display_name, protocol, endpoint, model, max_images, created_at, updated_at
	...
	return r.store.Write(ctx, "providers add", func(ctx context.Context, tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx,
			`INSERT INTO providers (id, display_name, protocol, endpoint, model, max_images, created_at, updated_at)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
			p.ID, p.DisplayName, p.Protocol, p.Endpoint, p.Model, p.MaxImages, at.Unix(), at.Unix())
		return err
	})
}
...
func (r *ProviderRepo) Update(ctx context.Context, id string, p Provider) error {
	...
	return r.store.Write(ctx, "providers update", func(ctx context.Context, tx *sql.Tx) error {
		res, err := tx.ExecContext(ctx,
			`UPDATE providers
			 SET display_name = ?, protocol = ?, endpoint = ?, model = ?, max_images = ?, updated_at = ?
			 WHERE id = ?`,
			p.DisplayName, p.Protocol, p.Endpoint, p.Model, p.MaxImages, at.Unix(), id)
		if err != nil {
			return err
		}
...
func scanProvider(row scanner) (Provider, error) {
	...
	if err := row.Scan(&p.ID, &p.DisplayName, &p.Protocol, &p.Endpoint, &p.Model, &p.MaxImages, &createdAt, &updatedAt); err != nil {
		return Provider{}, err
	}
