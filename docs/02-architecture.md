...
构建器能在没有 Mac、没有授权弹窗、没有真实帧的环境里测试。它是 M1 的交付物，不是 M4 的。
...
重新打开窗口的入口。**Wails 是否能承载这套语义是 M1 的硬门禁**，验证失败时停止 UI 扩张
并重新评估宿主（[风险 C-1](10-risks.md#c-1宿主无法承载后台-agent)）。

### 2.6.4 适配层监管

平台适配层若以独立进程形态实现（待定设计），则需要监管：健康检查、退避重启、重启后
重新下发配置并重放未确认帧。无论何种形态，**崩溃、断连或宿主重启都必须可恢复**。
