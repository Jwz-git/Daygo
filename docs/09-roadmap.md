...
| [delivery 安装与更新](modules/delivery.md) | 身份和分发实验、首次引导、安装、升级、安全重启 | 部分实现：GitHub Actions 已实现发布后自动构建、上传 macOS / Windows 安装器；签名 appcast 仅在显式 opt-in 且两端资产通过身份验证后生成；应用内 Sparkle / WinSparkle 适配器、设置 UI 和安全收尾已落盘 | Go / 前端 / appcast 夹具通过；发布工作流实现已核对；签名、公证、干净机安装与客户端升级仍缺正式证书材料，属 G-native 未验收 |
...
| 9 | 自动更新链路 | delivery / 工程 | **GitHub Actions 发布自动化已实现**：Release 发布后补齐两端安装器；签名 appcast 需显式 opt-in 且两端资产通过正式身份验证，普通发布只能手动安装。客户端仍采用 macOS Sparkle 2 / Windows WinSparkle + NSIS，见 [macOS 决策](decisions/delivery-auto-update.md)与 [Windows 决策](decisions/delivery-auto-update-windows.md)；真实升级状态单列于 delivery |
