...
| [data 数据管理与诊断](modules/data.md) | 数据库基础、锁、维护、磁盘限制、诊断和遥测开关 | 部分实现：跨平台 db-core、settings-store、diagnostics、checkpoint、备份与损坏恢复；存储设置页已接入 | macOS 单元与并发 smoke、Windows LockFileEx 跨进程 smoke、存储设置浏览器 smoke、DB-3/5/7 通过；1 小时 DB-8 运行中，清理与真实 Wails 重启未运行 |
