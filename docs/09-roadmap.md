...
> **最新验收确认（2026-09-26，基于 `ff573bf` 工作树）**：用户明确确认“所有已实现功能，含长期
> 观察和真实安装升级；未实现功能与正式证书缺失仍保留”。因此已实现能力（含近期宿主 / Dock /
> 菜单、隐藏媒体、内存改动、CLI / socket / MCP 与已实现平台能力）的功能验收统一记录为
> **用户确认已验收，未附逐项运行记录**。确认包含长期观察与现有身份下的真实安装升级，
> 不补写具体时长、系统 / 客户端版本、命令、测量值或逐项矩阵结果。
> 未实现功能、待定设计、Developer ID / 公证 / Authenticode 正式证书缺失保持原状态；
> 用户确认不等于正式 Gatekeeper / 签名身份认证。2026-09-22 和此前记录保留为历史证据。
...
| [delivery 安装与更新](modules/delivery.md) | 身份和分发实验、首次引导、安装、升级、安全重启 | 部分实现：GitHub Actions 安装器 / 签名 appcast、Sparkle / WinSparkle、安装前收尾、固定自签名身份与九语言 NSIS / 安装防护 | Go / 前端 / appcast / NSIS 夹具有记录；现有身份下已实现安装升级与客户端闭环于 09-26 用户确认已验收（无逐项记录）；正式 Developer ID / 公证 / Authenticode 材料仍缺，不将此确认记为正式身份验收 |
| [agent 对外程序化接口](modules/agent.md) | CLI 查询、agent.sock 受控写入、MCP 工具面 | 部分实现：CLI 读 / write 子命令、stdio MCP 五读六写、宿主 socket 与共享执行器、来源审计、MCP 配置 / CLI 示例；search 与 Windows 终端 CLI 未实现；v1 不交付 | Go 协议 / socket / 宿主与前端夹具有记录；已实现 CLI / socket / MCP 及真实客户端闭环于 09-26 用户确认已验收（无客户端版本 / 逐项记录）；未实现入口不在范围 |
| [chat 应用内对话](modules/chat.md) | 自然语言问答与沙箱内受控增删改查 | 部分实现：多会话纯对话、会话级 Provider / 模型、11 个封闭工具的 agent 循环、只读 / 只读实例双门禁、调用预算 / 取消、`llm_calls` 审计元数据和工具消息 UI | Go 回合 / 参数 / 门禁 / 预算 / 取消与前端夹具有记录；真实 Provider Wails 与已实现问答 / 编辑闭环于 09-26 用户确认已验收（无逐项记录）；search / status / 独立写入审计 / 诊断计数未实现，v1 不交付 |

### 9.1.1 本轮验收记录与证据边界

| 日期 / 代码范围 / 来源 | 确认内容 | 状态与限制 |
|---|---|---|
| 2026-09-26 / `ff573bf` 及此前已实现能力 / 本次用户明确回复 | 所有已实现功能，含长期观察和真实安装升级 | 用户确认已验收；未附逐项步骤、环境、时长或测量值。未实现功能、待定设计与正式证书缺失保持原状态；自动化失败 / 跳过 / 未运行记录不改写为通过 |

本轮是文档同步，不重新执行真实捕获、网络 Provider、MCP 客户端、安装升级或长期观察。
确认之后的新实现仍需独立记录增量证据，不把本次确认变成未来改动的自动验收。
各模块的历史运行记录保留原日期；用户确认是新增证据，不能倒填到原记录。
“已验收”限于已实现能力，不把部分实现的模块改成“实现齐备”。

2026-09-26 文档同步验证 / macOS arm64：`python3 scripts/check-docs.py` 检查 54 篇 Markdown，
0 处问题；`git diff --check` 无错误；`CGO_ENABLED=0 go test ./internal/storage ./internal/settings
./internal/timeutil ./internal/agentcli ./internal/agentbridge ./internal/mcp ./internal/chat` 通过
（复用 Go 缓存）；`npm --prefix frontend run test:unit` 169 项通过、0 失败 / 跳过。
这是本次文档核对证据，不代替原生 / 长期 / 安装升级实测；未重跑完整 `gate.sh`。

### 当前代码证据

**2026-09-26 Dock 报错增量修复**：原生激活策略根据实际读回值判断成功，重复设置不再误报；
modeless 提示在显示前布局，正文换行且仅保留配置按钮。Go 匿名设置往返、带可见窗口的独立
原生 / 匿名 `.app` smoke、universal 重建、强制重新链接与完整门禁通过。
本修复尚未进行真实 Wails 开关 / 重启与持续捕获回归，未替换已安装应用；不由此前用户确认
自动覆盖新修复。复现与证据详见 [recording](modules/recording.md)。
...
本轮已实现功能及长期观察于 09-26 用户确认已验收；未附本轮十分钟、24 小时 A/B 或 14 天 RSS
逐项记录与数值，不据此编造测量收益。复核流程见 [内存对照](08-testing-strategy.md#长期内存对照流程2026-09-26)。
...
- [storage](../internal/storage/)：连接与 PRAGMA 回读、迁移链（当前 v19，从 `app_settings`
  逐版增加 cards、recording、providers / chat、daily、analysis、分类种子、日报表、审查流、pending frame_index、分段均摊、多模型与摘要评分；v19 移除日记 summary）、
...
**原生实现与证据摘要**：macOS Capture / HEVC / Media 与 Windows DXGI/WGC / Media Foundation /
WIC 已接入共享 recorder、pending 对账与 `platform.Media`。已保留 macOS 真实像素、应用身份、
Windows 非黑 JPEG、窗口排除、实例锁和录制落库 smoke；详细设备、命令与限制分别见
[recording 执行册](modules/recording.md)及 [Windows 决策](decisions/recording-screen-capture-windows.md)。
两平台已实现能力和长期功能的最新状态为 09-26 用户确认已验收，未附完整逐项矩阵记录。
...
真实 G-host 回归与已实现关机 / 注销路径于 09-26 用户确认已验收（无逐项记录），证据见 [recording](modules/recording.md)。
...
DLL 匹配重建；macOS 原生匿名 smoke 通过，Windows 已实现能力经用户确认验收，DLL 重建与通知区的逐项命令结果未补录。
...
| 门禁 | 限制的工作 | 仍可推进 | 解锁证据 |
|---|---|---|---|
| G-host 宿主 | 大规模 UI 扩张、宣称常驻录制可用 | 限时一周的宿主探针、核心逻辑、契约与必要验证界面 | 真实 macOS：关窗后至少 10 分钟进程存活且**持续离散捕获**，状态栏重开、激活策略切换；IT-14；心跳仅是前置探针。09-22 历史验收与 09-26 增量用户确认均记录已验收（无逐项运行记录），大规模 UI 扩张解锁 |
| G-native 原生与身份 | 未决能力的大规模原生实现、对应正式身份声明 | 候选实验、fake、与实现形态无关的消费者 | 原生已实现能力、现有身份下授权 / 密钥及真实安装升级于 09-26 用户确认已验收（无逐项记录）；Developer ID / 公证 / Authenticode 材料仍缺，正式 Gatekeeper / 发布签名身份仍未验收 |
...
| G-stability 长期稳定性 | 宣称长时间 / 边界稳定性完成 | 模块交付、累计观察和修复 | 长期观察（含近期内存增量）于 09-26 用户确认已验收，无逐项时长 / 测量值；08 §8.6.5 的 14 天、DST、跨周与 A/B 仍作为复核要求，不据确认虚构各窗口数据 |
...
| 18 | Windows 发布范围 | delivery / 范围，recording 提供证据 | 已实现 DXGI/WGC、通知区 / 系统事件、锁、NSIS 与安装升级于 09-26 用户确认已验收（无逐项记录）；正式 Authenticode 材料仍缺，发布授权仍须用户明确给出。[WC](08-testing-strategy.md#863-wc真实-windows-捕获矩阵)、[WD](08-testing-strategy.md#864-wd真实-windows-分发矩阵)与 DB-8 保留为可复核矩阵，不倒填命令结果 |
...
| 22 | MCP 传输与进程模型（stdio 子进程 vs 宿主内 HTTP；工具粒度与审计来源标记随之一并定） | agent / 工程，delivery 协作 | **已决定并实现**：stdio `daygo mcp`，五读六写，读走只读 DB、写走 socket；来源封闭为 `agent.sock` / `cli` / `mcp`，只为审计标签。见 [决策](decisions/agent-mcp-transport.md)与 [05 §5.9.3](05-interface-contract.md#593-mcp-服务器)。已实现面与真实客户端闭环于 09-26 用户确认已验收（无客户端版本 / 逐项记录）；search 与 Windows 终端 CLI 未实现，v1 不交付 |
