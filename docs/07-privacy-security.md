...
`blocked` 且不调用截图。已有后台窗口排除 smoke；Windows 隐私双保护与 WC 矩阵经用户确认已验收，
未附逐项运行记录，状态与证据类型见 [09 §9.1](09-roadmap.md#91-模块总表)及
[决策记录](decisions/recording-screen-capture-windows.md)。用户确认不补写缺失的自动化结果。
...
- **默认关闭，opt-in。**
  当前仅有设置与隐私契约，实际遥测 / 崩溃上报消费者未实现，不能将设置保存视为上报链路验收。
...
| Agent socket（基础实现，v1 不交付） | 文件权限 `0600` 即访问控制；请求与响应各 1 MB 上限；所有输入视为不可信并做结构校验；`agentEditsEnabled` 关闭时返回 `edits_disabled`，且**服务端独立校验**，不信任客户端检查；成功写入审计只记时间 / 来源 / 操作 |
...
**卸载不等于删除用户数据。** Windows NSIS 卸载仅移除包拥有的文件、快捷方式和卸载注册表项，
保留应用支持目录与系统凭据；macOS 移除 `.app` 也不会自动删除应用支持目录和钥匙串条目。
一键彻底删除尚未实现，不得宣称卸载会清除全部副本。

备份不加密。数据库不含密钥（密钥在钥匙串），但含时间线内容，因此它继承目录的 `0700`
权限；卸载后随应用支持目录保留。备份不得写到该目录之外。
