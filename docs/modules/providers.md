...
重启后仍能判断密钥是否已配置。负责 U7、F-S1–4；支持现有 openai / anthropic 两种协议。
...
输出 provider-client，包含协议适配、路由、取消、重试与回退装饰器及文本调用能力。
...
1. 以固定配置、响应和错误夹具定义协议 / 路由 / 密钥行为；补 Secrets fake 契约。
2. 实现两种协议客户端、装饰器与取消，HTTP fixture 证明请求及错误路径；
   输出解析所需响应样本交给 timeline，避免两套重试规则。
...
完成要求：配置 / 路由 / 密钥全链路真实可用、两种协议夹具通过、真实配置服务连接通过、
无密钥泄漏；未配置时不发请求。fake 不证明外部服务或系统钥匙串可用。
...
HTTP、Secrets、真实服务及升级身份验证未运行；后续记录匿名夹具、commit 与环境。
