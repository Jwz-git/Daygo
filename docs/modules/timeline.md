...
2026-09-26：帧条新增 `/media/thumbnail?id=`，使用现有 Media 下采样端口输出最长边
256 像素 JPEG；主画面与放大查看维持完整单帧。资源仅接受一个数字 ID，数据库路径越界
返回 403；缺失资源返回 404。预加载器明确持有最多三个图片对象，切卡 / 卸载解除源与回调。
匿名资源夹具与预加载所有权夹具覆盖尺寸、请求参数、非法 ID、缺失资源、淘汰与清理；
`CGO_ENABLED=0 go test ./internal/app -run '^TestMedia'` 与 `./scripts/gate.sh` 通过。
已实现媒体改动及长期观察于 09-26 用户确认已验收；未附 WebContent / GPU 的固定场景测量值，缩略图不消除 HEVC 解码内部的全尺寸像素。

2026-09-26：图标 memo 在原条目上限之外增加载荷预算：Go favicon 8 MiB（图片字节），
前端 favicon 4 MiB、应用身份 8 MiB（字符串长度 × 2，应用身份含 ID / 名称 / 图标）。
覆盖 / 删除更新账目，超大条目仍返回给调用者但不 memo；淘汰不删除磁盘图标。
匿名夹具覆盖容量、字节淘汰、空值、覆盖、删除、超大结果、淘汰后磁盘重读与并发去重。
`go test -race ./internal/favicon`、前端单测及 `./scripts/gate.sh` 通过。
预算是缓存持有载荷的上界，不是进程 RSS 上限；长期观察于 09-26 用户确认已验收；未附 WebKit / GPU 降幅数值。
...
**当前边界**：分析帧读取经 app 的 `mediaFrameSource` 转发 `platform.Media`；资源处理器也经同一端口读取，
不再直接读取 staging 文件。卡片阶段互斥仍是全局而非按重叠范围。

**已知未修（需先决策再动）**：auth 批次 UI 标志说"不会自动重试"但 `RequeueFailed`
仍会重排（语义需决策）；Retry-After 无抖动（多组同限流时刻齐重试）；转录图片上限已按 Provider `maxImages` 接入；用户编辑被相邻批次回滚，需先对 docs/03 §3.5
明确重处理语义归属。
