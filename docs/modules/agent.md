...
多人视角。**v1 不交付**；基础实现已落，已实现面与真实客户端闭环于 09-26 用户确认已验收（无逐项记录）。
...
- 平台限制：Wails 在 Windows 生产构建中默认加 `-H windowsgui`，终端里运行得不到输出，
  ...
  已实现面于 09-26 用户确认已验收，未附管道 / AF_UNIX 的逐项真机记录。

未完成：search、Windows 终端 CLI、对外接口的诊断计数。真实 MCP 客户端闭环与长期观察
于 09-26 用户确认已验收，未附客户端版本与逐日记录。
...
|---|---|---|---|
| MCP 传输决策实验 | stdio 子进程（读只读 DB + 写走 socket）与宿主内 HTTP 两个最小探针：拉起、握手、一读一写 | 两候选的握手延迟、鉴权面、崩溃隔离可量化对比 | stdio 已按架构 / 隐私判据决定；HTTP 探针未实现，不声称有双候选量化对比；缺候选或回退即失败 |
