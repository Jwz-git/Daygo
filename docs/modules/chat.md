...
shell；不暴露原始帧、分段路径、密钥或 LLM payload；采用原子消息，token 级流式仅保留为后续候选；
...
chat 写入独立审计与诊断计数、消息留存策略（待定）。外部 socket 的 `agent-writes.log`
已由 agent 模块实现，但 chat 不经过 socket，尚未写入该日志。真实 Provider / Wails 闭环于 09-26
用户确认已验收（无逐项记录）。
...
|------|------|------|------|
| provider-client（`internal/ai` 统一 Generate、重试 / 回退、结构化输出） | providers | 回合状态机用脚本化 fake provider 做匿名夹具单测 | 已达成；chat 使用会话级 Provider / 模型，不走全局回退链 |
...
真实用户连续使用问答与受控编辑的闭环验收。UI 扩张与其他界面同样受 G-host 约束；G-host 与其依赖的真实 Provider 闭环已于 2026-09-22 经用户实测验收（无逐项运行记录），chat 已实现面及连续使用闭环于 09-26 用户确认已验收（无逐项记录），仍不进入 v1 交付范围。
