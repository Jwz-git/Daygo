...
Windows 录制目录迁移已增加纯 Go 复制、SHA-256 校验、数据库状态提交、重启后继续清理以及旧备份恢复门禁；自动化夹具通过。跨盘、断连及真实录制真机迁移尚无逐项运行记录，见[决策](../decisions/recording-directory-windows.md)。
...
切片 5 长期观察已于 2026-09-22 经用户实测验收（无逐项运行记录）。
...
录制清理已实现（按 `segment_path` 整段选择和删除），见下。
...
3. 在匿名分段 fixture 上实现 checkpoint、备份恢复与清理；真实 media-read 就绪后跑 IT-12。
   ...
   **checkpoint、备份、轮换、损坏恢复和按段录制清理均已落盘**；真实宿主 IT-12 与长期观察已于 2026-09-22 经用户实测验收（无逐项运行记录）。
4. 增加 GetDiagnostics 与匿名指标、磁盘和遥测设置、store 与 UI；当前已接入诊断 UI 和磁盘上限设置，
   遥测写入仍待实际 telemetry 消费者。
5. 累计 14 天磁盘 / 内存观察，与录制 / 更新共同验证关停和恢复；长期状态单列。长期观察已于 2026-09-22 经用户实测验收（无逐项运行记录，14 天 / 跨周窗口证据规则不变）。
...
清理按表内字节与状态工作，不做解码；Media 已接入帧条与分析读路径，其真机长期稳定性与
清理、分段恢复所受的 IT-12 / G-stability 已于 2026-09-22 经用户实测验收（无逐项运行记录）。
...
| 2026-09-13 / 见本次提交 / macOS arm64 · go1.26.3 · `CGO_ENABLED=0` | `go test -count=1 -run TestCleanup ./internal/storage/` | DB-9 的早期逐帧模型通过：上限收敛、租用 / 活跃保护、孤儿清扫和只读拒写均有断言 | 历史证据；不代表后续 HEVC 多帧分段已验收 |
| 2026-09-17 / HEVC 分段切片工作树 / macOS arm64 | storage 清理 / 迁移夹具与 `./scripts/gate.sh` | 按 `segment_path` 聚合整段软删除与物理删除、v16 大小均摊、活跃段和租用段保护通过自动化 | 真实宿主 IT-12 与长期磁盘曲线仍未运行 |
| 2026-09-26 / Windows 录制目录迁移 | 目录迁移 Go 定向测试与 `-race`、`CGO_ENABLED=0 go build ./...`、`go vet ./...`、前端 build / 169 项单测、`scripts/check-docs.py` | 全部通过；复制校验、重试、所有权、录制状态恢复与旧备份恢复门禁有匿名夹具 | `go test ./internal/...` 的 Windows 运行仍有现存 `/tmp` 路径与原生 ABI 测试失败；真实跨盘、拔盘与长时录制迁移未验收 |

**关键结果**：并发备份曾因秒级文件名冲突而失败，现以互斥区内单调序号命名并有回归测试。DB-3 空库 / 旧库只读、DB-5 metadata 往返、DB-8 一小时并发（3600.11 秒，零忙锁风暴、完整性检查为 `ok`）与现行 DB-9 按段清理均有自动化证据。IT-12 真实宿主与长期磁盘曲线由用户于 2026-09-22 确认验收，未附逐项运行记录。
