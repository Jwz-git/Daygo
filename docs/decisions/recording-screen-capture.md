# recording 屏幕捕获：技术调研、数据契约与原生 ABI

> **状态：recording 实验规格。** 本文冻结上层捕获数据契约和 ABI v1 的边界，给出 macOS 的首选实现与
> Windows 的候选实现。它不等于完成 [原生适配能力决策](../06-native-integration.md#66-选型时要回答的问题)：
> 真实授权、捕获指示、隐私遮蔽、签名、公证和长时间运行实验通过前，`09` 中的平台实现仍是
>「待定设计」。
>
> **当前代码事实。** `internal/platform` 端口、值类型与封闭集校验已落盘（接口，无实现）；
> `internal/app` 的录制 / 权限及公共绑定骨架已挂入 Wails `Bind`（权限方法在真实适配层落地前返回
