...
> **状态：已落盘的打包脚本 + CI 切片（三条签名路径、自签名证书生成 / 导出、GitHub Actions keychain 导入）。
> 真机重启 / 升级身份、CI 持久化跨版本 DR 一致、干净机器 Gatekeeper 与公证可行性仍受 G-native 门禁约束，
> 缺可复核证据时不得宣称「已支持」。**
...
**决定**：采用**持久化自签名稳定证书**。理由：无需 Apple 付费账号即可获得稳定 DR，根治「更新 / 重构后
授权失效」。它**不公证、不用于分发**（Gatekeeper 仍提示未识别开发者，自用可接受）；对外正式发布仍走
Developer ID + 公证（已排期，属 G-native）。
...
- **导出证书材料**（一次性，本地）：`DAYGO_DEV_CERT_P12_OUT=/tmp/daygo-ci.p12 ./scripts/dev-cert-macos.sh`
  在生成 / 导入证书的同时，额外导出一份**带口令的 p12**，并打印其 base64 与随机口令。脚本拒绝把 p12 写进
  仓库工作树（私钥绝不进 Git 跟踪的文件）；用户复制后应立即删除该文件。
- **GitHub Secrets**（用户手动配置）：`DAYGO_DEV_CERT_P12_BASE64`、`DAYGO_DEV_CERT_P12_PASSWORD`。已有的
  `SPARKLE_ED25519_PRIVATE_KEY`（更新信任锚）不变。
- **workflow**（[`publish-release.yml`](../../.github/workflows/publish-release.yml) 的 macOS job）：打包前新增
  "Import signing certificate" 步骤——secret 非空则建临时 keychain、base64 解码 p12、`security import
  -T /usr/bin/codesign`、`set-key-partition-list`、加入搜索列表，并把 `DAYGO_DEV_SIGN_IDENTITY=Daygo Dev`
  写进 `$GITHUB_ENV`，`package-macos.sh` 随即走自签名分支。secret 未配置则整步 no-op → 回退 ad-hoc
  （fork 无证书也能出包）。`always()` 步骤删除临时 keychain。EdDSA / appcast 步骤不受影响。
...
- **CI 持久化验收（未验证）**：`codesign -dr - Daygo.app` 确认 DR 为 `identifier … and certificate leaf …`
  （非 cdhash）且两版一致；升第二版后 Sparkle 更新**不重新授权**即可继续录制；Console.app 无 Sparkle 代码
  签名拒绝。须在真实 macOS + 测试 appcast 上走一轮，属 G-native，不产出正式 release 产物。
