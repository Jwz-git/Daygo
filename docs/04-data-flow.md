...
**同次输出的短末卡也折叠。** 若模型一次返回多张卡，末卡仍受 15 分钟下限豁免，但此前两道
折叠只检查输出的第一张卡，会使紧邻上一张输出的短末卡在首次生成时留存、单卡重生成时却被合并。
现在在检查已落库前驱之前，先将 `< 13 分钟`的末卡按同一邻接、分类排除与 60 分钟跨度闸门
并入本次输出的倒数第二张卡；两卡身份与 metadata 沿用 `buildMergedShell` 的多数时间规则。
无合格邻卡时仍保留末卡；13–15 分钟的末卡仍享有豁免。批次生成与单卡重写共用这一步。
...
用户可从失败面板**手动停止自动重试**（`StopRetries` 绑定，[05 §5.5.1](05-interface-contract.md#551-绑定方法目录)）：
它把批次的 `attempts` 封顶到 `MaxBatchAttempts`——正是冷却循环自行放弃时批次到达的状态，
于是 `RequeueFailed` 跳过它、面板不再宣称「将自动重试」。批次保持失败终态且可见，不删行、
不发 LLM 调用；之后 `RetryBatches` 仍会重置计数覆盖它，因此停止可逆。
