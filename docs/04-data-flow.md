...
之前补一道确定性闸门：当本批次输出**恰好一张卡且时长 < 10 分钟**时，若它前面紧邻一张已提交
...
**紧跟一张跨分类邻居**的短卡。参数：`shortSingleCardCeiling = 10min`、`maxShortCardMergeGap = 4min`、
...
flowchart TD
    ...
    V -->|合法| PIN["夹紧外边界 → ReplaceCardsInRange(start, end)"]
```
...
- **窗口由卡片自己的 `start` / `end` 决定**，不是来源批次的范围。这也正是修复点：一次被 60
  ...
  重新生成其中任意一张都只动那一张。
- **两侧外边界不动。** 提示词明确要求只重写当前窗口；模型越界时校验驳回，最终写入前
  `shells[0].Start` 与 `shells[len-1].End` 再夹紧回原卡片的边界，因此相邻卡片永远不需要
  被吸收或让位。窗口内仍允许分成多张卡（受 [§4.3.1](#431-分批规则) 的 15 分钟下限约束）。
- **越界即失败，不降级写入。** 三次校正后仍不合法时返回错误、**不落任何改动**——重新生成
  的可预期结果只有「这几张新卡」或「原样不动」，不存在"改了一半"。
