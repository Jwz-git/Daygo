...
- **融合有确定性闸门，不依赖模型自律。** 模型声明的融合（输出卡 start 早于批次窗口）
  ...
  **横跨批次起点的那张前卡不受闸门约束**：批次证据与它重叠，它以 start 无条件成为
  ...
  分类的，是 [§4.3.1](#431-分批规则) 的单卡短卡恢复性融合**：它不是模型声明的融合，而是生成
  完成后的一道独立确定性步骤，只在输出恰好一张短卡时触发，按多数时间分类取身份并把前驱纳入
  改写所有权——因此两者不冲突，本闸门仍拒绝模型自己声明的跨分类吞并。
...
- **窗口由卡片自己的 `start` / `end` 决定**，不是来源批次的范围。这也正是修复点：一次被 60
  ...
  重新生成其中任意一张都只动那一张。
- **右外边界永不动，左外边界仅在短单卡融合时外扩。** 提示词明确要求只重写当前窗口；模型
  越界时校验驳回，最终写入前 `shells[0].Start` 与 `shells[len-1].End` 夹紧回原卡片的边界。
  **唯一例外**是输出恰好一张 < 13 分钟的卡：此时复用 [§4.3.1](#431-分批规则) 的单卡短卡融合闸门，
  把它并入紧邻的已提交前驱——左边界随之外移到该前驱起点，`ReplaceCardsInRange` 因而拥有并
  替换这张前驱（多数时间分类取身份，与批次流水线同一套 `mergeableSingleCardPredecessor` /
  `buildMergedShell`）。若该前驱跨度正被 pending / processing 批次占用则跳过融合、退回纯窗口内
  重写，避免那条批次落库时把融合结果覆盖掉。除此之外相邻卡片不被吸收或让位；窗口内仍允许
  分成多张卡（受 [§4.3.1](#431-分批规则) 的 15 分钟下限约束）。
