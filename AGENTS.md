...
遵循 `docs/09-roadmap.md`：

| M | 目标 | 退出条件 |
|---|------|----------|
| 0 | 桌面外壳与前端骨架 | 能启动、路由、切主题与语言，设置能持久化 |
| 1 | 宿主能力验证 + platform 端口 + fake | **证明宿主能承载后台 Agent**，并落盘适配层选型决策 |
| 2 | 存储与设置 | schema、迁移、设置与 provider 全链路可用，绑定层接管前端本地存储 |
| 3 | 捕获与分析 | 连续 7 天自用，时间线由真实捕获与分析产生 |
| 4 | v1 界面完整 | 时间线 + 每日 + 每周 + 全部设置分区可日常使用 |
| 5 | 发布 | 签名、公证、自动更新链路打通 |

**M1 是硬门禁。** 在证明"关闭窗口后进程存活并继续捕获"之前，不要投入大规模 UI 开发——
那是一个前提风险，不是实现细节（`docs/10-risks.md` C-1）。
...
- 无窗口常驻、accessory 激活策略、状态栏和自动更新是 M1 硬门禁；验证失败时停止 UI 扩张
  并重新评估宿主架构。
...
完成任务时报告：实现内容、对应里程碑门禁、运行命令及结果、未验证风险和回退方式。
