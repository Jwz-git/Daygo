...
const blockedApplicationIds = computed(() => blockedApplicationIdsText.value
  .split(/[,\n]/u)
...
function errorMessage(cause: unknown): string {
  return cause instanceof Error && cause.message === WAILS_UNAVAILABLE
...
async function openFolder(): Promise<void> {
  ...
  try {
    await openCaptureTestFolder(results.value[0].outputPath)
  } catch (cause: unknown) {
    error.value = errorMessage(cause)
  }
}
...
function formatResult(result: CaptureTestResult): string {
  ...
  return t('captureTest.result.written', { width: result.width, height: result.height, bytes: result.fileSize })
}
...
async function pollEvents(): Promise<void> {
  try {
    ...
    for (const event of events) {
      if (['sleep', 'screen_locked', 'screensaver_start'].includes(event.kind)) systemPaused.value = true
      else if (['wake', 'screen_unlocked', 'screensaver_stop'].includes(event.kind)) systemPaused.value = false
    }
  } catch {
    // The Wails bridge is optional in the Vite preview.
  }
}
...
<template>
  ...
  <section class="dg-card form-card" :aria-label="t('captureTest.configuration')">
    ...
    <div class="grid">
      <label class="field">
        ...
        <input v-model.number="targetHeight" class="dg-input" type="number" min="1" max="16384">
      </label>
      <label class="field">
  ...
  <section class="dg-card form-card" :aria-label="t('captureTest.schedule')">
    ...
    <div class="grid">
      <label class="field"><span class="dg-field-label">{{ t('captureTest.intervalSeconds') }}</span><input v-model.number="intervalSeconds" class="dg-input" type="number" min="0"></label>
      <label class="field"><span class="dg-field-label">{{ t('captureTest.durationSeconds') }}</span><input v-model.number="durationSeconds" class="dg-input" type="number" min="1"></label>
    </div>
    <p class="hint">{{ t('captureTest.scheduleHint') }}</p>
    <div class="actions">
      <button class="dg-button dg-button--primary" :disabled="busy || running" type="button" @click="captureOnce">{{ t('captureTest.captureOnce') }}</button>
      <button class="dg-button" :disabled="running" type="button" @click="startSchedule">{{ t('captureTest.startSchedule') }}</button>
      <button class="dg-button" :disabled="results.length === 0" type="button" @click="openFolder">{{ t('captureTest.openFolder') }}</button>
      <button class="dg-button" :disabled="!running" type="button" @click="stopSchedule">{{ t('captureTest.stopSchedule') }}</button>
    </div>
  </section>
  ...
  <section v-if="results.length > 0" class="dg-card results" :aria-label="t('captureTest.results')">
    ...
    <ol><li v-for="result in results" :key="result.outputPath"><strong>{{ formatResult(result) }}</strong><code>{{ result.outputPath }}</code></li></ol>
  </section>
</template>

<style scoped>
.intro,.hint{color:var(--dg-text-secondary);font-size:13px;line-height:1.5}.dg-card{padding:20px}h2{margin:0 0 18px;color:var(--dg-text-primary);font-size:15px}.form-card,.field{display:grid;gap:16px}.field{gap:7px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.textarea{resize:vertical}.actions{display:flex;flex-wrap:wrap;align-items:center;gap:10px}.check{display:flex;align-items:center;gap:8px;color:var(--dg-text-secondary);font-size:13px}.error{padding:12px 14px;border:1px solid var(--dg-danger);border-radius:10px;color:var(--dg-danger);font-size:13px}.results ol{display:grid;gap:12px;margin:0;padding-left:20px}.results li{display:grid;gap:4px;color:var(--dg-text-secondary);font-size:13px}.results strong{color:var(--dg-text-primary)}.results code{overflow-wrap:anywhere;color:var(--dg-text-tertiary);font-family:var(--dg-font-mono);font-size:12px}@media(max-width:680px){.grid{grid-template-columns:1fr}}
</style>
